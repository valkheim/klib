...
namespace kl
{
  ...
  // if they require a low-overhead form of mutual exclusion for code that runs at
  // IRQL <= APC_LEVEL.
  ...
  class FastMutex final : public ILock
  {
   ...
   public:
    ...
    _IRQL_requires_max_(DISPATCH_LEVEL) void Init();

    _IRQL_raises_(APC_LEVEL) _IRQL_saves_global_(OldIrql, mutex) void Lock();

    _IRQL_raises_(APC_LEVEL) _IRQL_saves_global_(OldIrql, mutex) _Success_(return == true) [[nodiscard]] auto TryLock() -> bool;

    _IRQL_requires_(APC_LEVEL) _IRQL_restores_global_(OldIrql, mutex) void Unlock();
  };
  ...
  class GuardedMutex final : public ILock
  {
   ...
   public:
    ...
    _IRQL_requires_max_(APC_LEVEL) void Lock();

    _IRQL_raises_(APC_LEVEL) _IRQL_saves_global_(OldIrql, mutex) _Success_(return == true) [[nodiscard]] auto TryLock() -> bool;

    _IRQL_requires_max_(APC_LEVEL) void Unlock();
  };

  // Spin locks are kernel-defined, kernel-mode-only synchronization mechanisms,
  // exported as an opaque type: KSPIN_LOCK.
  // A spin lock can be used to protect shared data or resources from simultaneous
  // access by routines that can execute concurrently and at IRQL >=
  // DISPATCH_LEVEL in SMP machines.
  ...
  class SpinLock final : public ILock
  {
   ...
   public:
    ...
    _IRQL_saves_global_(SpinLock, old_irql) _IRQL_raises_(DISPATCH_LEVEL) void Lock();

    _IRQL_requires_(DISPATCH_LEVEL) _IRQL_restores_global_(SpinLock, old_irql) void Unlock();
  };

  // Queued spin locks are a variant of spin locks that are more efficient for
  // high contention locks on multiprocessor machines. On multiprocessor machines,
  // using queued spin locks guarantees that processors acquire the spin lock on a
  // first - come first - served basis. Drivers for Windows XP and later versions
  // of Windows should use queued spin locks instead of ordinary spin locks.
  ...
  class QueuedSpinLock final : public ILock
  {
   ...
   public:
    ...
    _IRQL_requires_max_(DISPATCH_LEVEL) _IRQL_saves_global_(QueuedSpinLock, handle) _IRQL_raises_(DISPATCH_LEVEL) void Lock();

    _IRQL_requires_(DISPATCH_LEVEL) _IRQL_restores_global_(QueuedSpinLock, handle) void Unlock();

    _IRQL_requires_(DISPATCH_LEVEL) void LockAtDpc();

    _IRQL_requires_(DISPATCH_LEVEL) void UnlockFromDpc();
  };
  ...
  class ExecutiveResource : public ILock
  {
   ...
   public:
    ...
    _IRQL_requires_max_(APC_LEVEL) ~ExecutiveResource();

    _IRQL_requires_max_(DISPATCH_LEVEL) void Init() override;

    _IRQL_requires_max_(DISPATCH_LEVEL) auto Reinitialize() -> NTSTATUS;

    _IRQL_requires_max_(APC_LEVEL) void Lock() override;

    _IRQL_requires_max_(APC_LEVEL) auto LockShared() -> BOOLEAN;

    _IRQL_requires_max_(DISPATCH_LEVEL) void Unlock() override;
  };
}  // namespace kl